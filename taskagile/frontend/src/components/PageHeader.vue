    ...
    <div class="boards-menu-toggle">
      <div class="dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="boardsMenu">
          <div v-if="!hasBoards" class="dropdown-item">{{ $t('header.boardsMenu.noBoard') }}</div>
          <div v-if="hasBoards">
            <h6 class="dropdown-header" v-if="personalBoards">{{ $t('header.boardsMenu.personalBoards') }}</h6>
...
export default {
  name: 'PageHeader',
  computed: { // Getter를 계산된 프로퍼티로 추가하고, 템플릿에서 사용한다.
